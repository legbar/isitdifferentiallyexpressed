...
source("modules/customDataset.R")
...
sidebar <- dashboardSidebar(width = 300,
  sidebarMenu(
    ...
    menuItem("Custom Dataset", tabName = "customDataset", icon = icon("table"))
  )
)
 
 body <- dashboardBody(
  tabItems(
    tabItem(tabName = "customDataset",
            customDatasetUI("customDataset")),
...
server <- function(input, output, session) {
  ...
  customDatasetServer("customDataset")
